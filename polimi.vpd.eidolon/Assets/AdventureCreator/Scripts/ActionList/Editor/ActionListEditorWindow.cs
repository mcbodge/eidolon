...
namespace AC
{
	...
	public class ActionListEditorWindow : EditorWindow
	{
		...
		private void TopToolbarGUI (bool isAsset)
		{
			...
			GUILayout.BeginArea (new Rect (0, position.height - 20, position.width, 20), nodeSkin.box);
			string labelText;
			if (noList)
			{
			...
			if (GUI.Button (new Rect (10, 0, 18, 18), "", nodeSkin.customStyles [iconNumber]))
			{
		...
		public static GUISkin emptyNodeSkin
		{
			get
			{
				...
				else
				{
					...
					{
						ACDebug.LogWarning ("Cannot find GUISkin asset file 'Assets/AdventureCreator/Graphics/Skins/ACEmptyNodeSkin.guiskin'");
					}
